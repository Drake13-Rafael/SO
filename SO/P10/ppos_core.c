...
void dispatcher(){
    ...
    while((queue_size((queue_t*) fila_tasks) > 0)||semaforos_existem){ // Enquanto houver tarefas de usuário
...
int task_wait(task_t *task){
    ...
    #ifdef DEBUG
        printf("Tarefa %d suspensa pela tarefa %d\n", task_atual->id, task->id);
    #endif
...
void acordar_tarefas_semaforo(task_t *fila){
    ...
    aux = aux->next;
    #ifdef DEBUG
        printf("tamanho da fila na hora de acordar: %d\n", queue_size((queue_t *) fila));
    #endif
    if(fila){
        do{
            task_resume(aux->prev, &fila);
            aux = aux->next;
        }while(fila_adormecidas != NULL);
    }
}
...
int sem_down(semaphore_t *s){
    ...
    #ifdef DEBUG
        printf("Derrubando semaforo s%d pra tarefa %d\n", s->id, task_atual->id);
    #endif

...
task_t t_main, *task_atual, t_dispatcher, *fila_tasks, *fila_suspensas;
...
void dispatcher(){
    ...
    while(queue_size((queue_t*) fila_tasks) > 0){ // Enquanto houver tarefas de usuário
...
int task_wait(task_t *task){
    ...
    fprintf(stderr, "##Erro de tarefa encerrada\n");
    return -1;
}
